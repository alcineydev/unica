...
export async function POST(request: NextRequest) {
  logger.debug('[UPLOAD] ====== INICIO ======')

  try {
    // Verificar autenticação
    const session = await auth()
    logger.debug('[UPLOAD] Session:', session?.user?.email || 'não autenticado')

    if (!session || !['DEVELOPER', 'ADMIN', 'PARCEIRO', 'ASSINANTE'].includes(session.user.role)) {
      logger.debug('[UPLOAD] Não autorizado - role:', session?.user?.role)
    ...
    logger.debug('[UPLOAD] File:', file?.name, file?.size, file?.type)
    ...
    if (!publicUrl) {
      logger.debug('[UPLOAD] Falha no upload para Cloudinary')
      return NextResponse.json({
        error: 'Erro ao fazer upload. Verifique as configurações do Cloudinary.'
      }, { status: 500 })
    }

    logger.log('[UPLOAD] ====== SUCESSO ======')
    return NextResponse.json({ 
      success: true,
      url: publicUrl
    })
