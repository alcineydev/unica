...
4. `src/components/admin/sidebar.tsx`
   - Sidebar colapsável com transição suave (`w-64` ↔ `w-20`), tooltips em modo colapsado, toggle no rodapé, labels/chevrons condicionais, e popover fixed já existente preservado.

5. `src/components/admin/header.tsx`
   - Mantido alinhamento `justify-end` e aria-label em notificações (sem alterações de estrutura adicionais nesta etapa).
